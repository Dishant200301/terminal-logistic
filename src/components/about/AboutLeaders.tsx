...
const LeaderCard = ({ leader, index, total }: { leader: typeof leaders[0], index: number, total: number }) => {
  const [isClicked, setIsClicked] = useState(false);

  // Responsive edge detection for applying fading outer grid lines natively matching the design image
  const isLeftLg = index % 3 === 0;
  // Leftmost card on bottom row (index 6 for length 8)
  const isBottomLeftLg = index === 6; 
  ...
  return (
    ...
    >
      ...
      {/* Card Content - sits inside, creating the "border" gap */}
      <div className="card-content relative bg-white flex flex-col">
        {/* Background glow div that sits behind the opaque image wrapper */}
        ...
        <div className="relative w-full p-6 pb-0 md:p-8 md:pb-0 z-10">
          ...
          <div className="relative w-full aspect-square overflow-hidden">
            ...
            >
              {Array.isArray(leader.details) ? (
                leader.details.map((paragraph, idx) => (
            ...
            >
              <img src={leader.img} alt={leader.name} className="w-[101%] h-[101%] max-w-none -ml-[0.5%] -mt-[0.5%] object-cover pointer-events-none" />
        ...
        <div className="relative w-full py-[24px] mt-6 flex flex-col items-center justify-center border-t border-[#EAEAEA] bg-white z-20">
          ...
          {/* Fading Extended Infinite Grid Lines (matched exactly to Image) */}
          {/* Far Left line extending horizontally outward */}
          {isLeftLg && (
          ...
          {/* Far Left Bottom section vertical downward falloff line */}
          {isBottomLeftLg && (
          ...
          <h3 className="text-[#052424] text-[22px] font-sans font-medium mb-[2px] tracking-tight">{leader.name}</h3>
          <p className="text-[#A0A0A0] text-[15px] font-sans font-normal">{leader.role}</p>
        </div>
      </div>
    </div>
  );
...
const AboutLeaders = () => {
  ...
  return (
    <section className="py-24 md:py-32 bg-white">
      <div className="site-container max-w-[1250px]">
        <AnimatedSection className="text-center mb-16 md:mb-24 px-6 md:px-0 flex flex-col items-center">
          <p className="text-[20px] text-[#C2C2C2] font-normal leading-[19px] mb-4 md:mb-8 font-sans">Our Leaders</p>
          ...
          <div className="font-sans flex flex-col items-center text-center">
            <p className="text-[18px] text-[#7F7F7F] font-normal leading-[25px] mb-6 md:mb-8">
              Shipper leaders drive towards a combined mission with <br className="hidden md:inline" />
              extreme ownership, smart execution, and passionate <br className="hidden md:inline" />
              innovation.
            </p>
            <p className="text-[18px] text-[#7F7F7F] font-normal leading-[25px]">Meet the strategic problem solvers at the helm:</p>
          </div>
        </AnimatedSection>
        ...
        >
          {leaders.map((leader, index) => (
            <LeaderCard key={leader.name} leader={leader} index={index} total={leaders.length} />
          ))}
          {/* Prevent native grid gap from creating large grey blocks on empty cells (e.g. 9th spot on desktop) */}
          {Array.from({ length: leaders.length % 3 === 0 ? 0 : 3 - (leaders.length % 3) }).map((_, i) => (
            <div key={`empty-lg-${i}`} className="hidden lg:block relative bg-white w-full h-full pointer-events-none z-20">
              {/* Mask to precisely erase the grid's bounding border exclusively on the empty 9th cell's right wing */}
              <div className="absolute -top-[1px] -right-[2px] w-[3px] h-[calc(100%+2px)] bg-white" />
              {/* Mask to completely erase the container's bottom bounding edge beneath the 9th cell */}
              <div className="absolute -bottom-[2px] -left-[1px] w-[calc(100%+3px)] h-[3px] bg-white" />
            </div>
          ))}
          {Array.from({ length: leaders.length % 2 === 0 ? 0 : 2 - (leaders.length % 2) }).map((_, i) => (
